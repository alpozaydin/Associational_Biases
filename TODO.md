...
- [ ] **Wk4c step 1: rerun Wk4b with `--seed 1`.** Blocks everything downstream. If seed=1 lands ~74% with no fallback → mechanism confirmed. If it collapses → hair+bg just happened to land in a non-collapsed basin on seed 0, reframe again.
- [ ] **Wk4c step 2 (after seed=1 confirms): demographic-slice acc.** FairFace / open_clip / DeepFace on 280 probe imgs; per-slice acc gap stock vs Wk4b. "Acc improved" ≠ "bias reduced" — this is the actual project deliverable. If gain is uniform across gender/race slices, we have a better emotion classifier and no bias story. If concentrated in underrepresented slice, that's the paper.
- [ ] **SKIP** `llm_projector` under simsiam+hair+bg (per advisor: headroom question, wrong priority — placement escalation only makes sense if seed variance confirms the current mechanism *and* demographic slice shows a gap to close).
