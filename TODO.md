...
- [ ] **Wk4b: rerun simsiam with `--redact-mode hair+bg`.** Kill the bg fallback so face becomes the only cue that survives both perturbations. Mechanism valid for emotion (GT is bg-independent). Same trainer, config change only. Predict Δface > 0.
