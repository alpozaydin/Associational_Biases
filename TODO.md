...
- [ ] **Wk4c direction (decide next):** (A) `hair+skin` grayscale — orthogonal color-cue perturbation, preserves geometry; (B) drop λ to ~0.3 so task NLL dominates; (C) escalate placement to `llm_projector` so LoRA has direct handle on vision tokens; (D) sample redaction mode per step (hair/hair+skin/hair+bg) — enforce invariance across cues, not one. Tradeoff: D principled but 3× compute; A cheapest with clear mechanism.
