...
- [x] **Wk4c pre-work — probe acc + confusion sweep (advisor-mandated):** Ground-truth signal for adaptation quality. Results on 280-img probe:
  ...
  - **Story reframe**: simsiam consistency is a REGULARIZER not just an invariance loss. Hair-only redaction (weak) recovers stock; hair+bg (strong) prevents overfit + breaks decoder-fallback pathology. Wk4b CAM "regression" (Δface=-0.020) reflects diffuse vision attention in a better classifier, not routing to bg content.
- [ ] **Wk4c direction (open):** Now blocked on demographic-slice evaluation, not more CAM/redaction knobs. Options: (A) FairFace / open_clip / DeepFace to per-slice acc on probe — needed for "bias mitigation" claim; (B) confirm Wk4b with `--seed 1` (was in original plan, now cheaper since we have per-class + confusion diagnostics); (C) placement escalation `llm_projector` under simsiam+hair+bg (Wk1 rejected it under task-only, different loss now — might push acc further or diagnose a plateau). Tradeoff: (A) is what actually validates the project claim; (B) rules out seed-luck (Wk4b's non-collapse could be seed-0-specific); (C) explores headroom but doesn't answer the project question.
